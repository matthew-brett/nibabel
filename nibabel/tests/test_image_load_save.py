    ...
    for npt in np.float32, np.int16:
        ...
        for r_class_def in class_map.values():
            ...
            for w_class_def in class_map.values():
                ...
                assert_array_equal(img2.get_affine(copy=True), affine)
...
def test_save_load_endian():
    ...
    assert_equal(img.get_header(copy=True).endianness, native_code)
    img2 = round_trip(img)
    assert_equal(img2.get_header(copy=True).endianness, native_code)
    assert_array_equal(img2.get_data(), data)
    # byte swapped endian image
    bs_hdr = img.get_header(copy=True).as_byteswapped()
    bs_img = Nifti1Image(data, affine, bs_hdr)
    assert_equal(bs_img.get_header(copy=True).endianness, swapped_code)
    ...
    # this will have made the header native by doing the header conversion
    cbs_hdr = cbs_img.get_header(copy=True)
    ...
    cbs_hdr2 = cbs_img2.get_header(copy=True)
    ...
    assert_equal(bs_img2.get_header(copy=True).endianness, swapped_code)
    ...
    assert_equal(mixed_img.get_header(copy=True).endianness, native_code)
    m_img2 = round_trip(mixed_img)
    assert_equal(m_img2.get_header(copy=True).endianness, native_code)
...
def test_save_load():
    ...
    with InTemporaryDirectory() as pth:
        ...
        assert_array_equal(re_img.get_affine(copy=True), affine)
        ...
        if have_scipy: # skip we we cannot read .mat files
            ...
            assert_array_equal(re_img2.get_affine(copy=True), affine)
            del re_img2
            spm99.save(img, sifn)
            ...
            assert_array_equal(re_img3.get_affine(copy=True), affine)
...
def test_two_to_one():
    ...
    # single file format
    img = ni1.Nifti1Image(data, affine)
    assert_equal(img.get_header(copy=True)['magic'], asbytes('n+1'))
    str_io = BytesIO()
    img.file_map['image'].fileobj = str_io
    # check that the single format vox offset is set correctly
    img.to_file_map()
    assert_equal(img.get_header(copy=True)['magic'], asbytes('n+1'))
    assert_equal(img.get_header(copy=True)['vox_offset'], 352)
    # make a new pair image, with the single image header
    pimg = ni1.Nifti1Pair(data, affine, img.get_header(copy=True))
    isio = BytesIO()
    hsio = BytesIO()
    ...
    assert_equal(pimg.get_header(copy=True)['magic'], asbytes('ni1'))
    assert_equal(pimg.get_header(copy=True)['vox_offset'], 352)
    ...
    assert_equal(ana_img.get_header(copy=True)['vox_offset'], 352)
    ...
    assert_equal(aimg.get_header(copy=True)['vox_offset'], 352)
    aimg = spm99.Spm99AnalyzeImage.from_image(img)
    assert_equal(aimg.get_header(copy=True)['vox_offset'], 352)
    aimg = spm2.Spm2AnalyzeImage.from_image(img)
    assert_equal(aimg.get_header(copy=True)['vox_offset'], 352)
    nfimg = ni1.Nifti1Pair.from_image(img)
    assert_equal(nfimg.get_header(copy=True)['vox_offset'], 352)
    # now set the vox offset directly (modifying the header in-place
    hdr = nfimg.get_header(copy=False)
    hdr['vox_offset'] = 0
    assert_equal(nfimg.get_header(copy=True)['vox_offset'], 0)
